...
func RouteDigest(digest *chi.Mux, dbInstance *sql.DB) *chi.Mux{
	...
	digest.Delete("/category/prime/{CatPrimeID}",rCatagories.DeletePrimeCategory)
	digest.Delete("/category/sub/{CatSubID}",rCatagories.DeleteSubCategory)
	digest.Delete("/category/final/{CatFinalID}",rCatagories.DeleteFinalCategory)
	...
	digest.Delete("/variation/{VariationID}",rAdmin.DeleteVariation)
	...
	digest.Put("/product-size/{ProductSizeID}",rAdmin.EditProductSize)
	...
	digest.Delete("/wishlists/{wishlistID:[0-9]+}", rWishlist.DeleteWishlistEndpoint)
	...
	digest.Delete("/users/profile",rUser.DeleteUserProfileEndpoint)
	return digest
